...
path = r'D:\sourc_code\code_IRI\data_test\survey_data\20220608'                             #str(input('ที่อยู่ไฟล์='))
file = str(input('ชื่อไฟล์='))
#run = str(input('หมายเลขRun='))

pyodbc.lowercase = False
path_mdb = '%s\\%s.mdb' % (path,file)
...
######################## Fetching Data in Microsoft Access  ############################################################################################################
...
with open('%s\\%s\\access_valuelaser.csv' % (path,a), 'w', newline='') as f:
    writer = csv.writer(f)
...
cre_key = '''SELECT CHAINAGE_START as event_str, CHAINAGE_END as event_end, EVENT AS event_num, 
        ...
        LONGITUDE_END as lon_end ,ramp_id
        from KeyCode_Raw_%s''' % (a)
cur = conPG.cursor()
sql = cre_key
cur.execute(sql)

with open('%s\\%s\\access_key.csv' % (path,a), 'w', newline='') as f:
    writer = csv.writer(f)
    for row in cur.fetchall():
        writer.writerow(row)
cur.close()
...
with open('%s\\%s\\access_pic.csv' % (path,a), 'w', newline='') as f:
    writer = csv.writer(f)
    for row in cur.fetchall():
        writer.writerow(row)
cur.close()
######################################################################################################################################
cre_pave = '''select CHAINAGE as chainage_pic, FRAME as  frame_number , X as lon, Y as lat
        from Video_Processed_%s_2''' % (a)

cur = conPG.cursor()
sql = cre_pave
cur.execute(sql)

with open('%s\\%s\\access_distress_pic.csv' % (path,a), 'w', newline='') as f:
    writer = csv.writer(f)
    for row in cur.fetchall():
        writer.writerow(row)
cur.close()
...
print("STEPconnect:Database Connect successfully")
...
CREATE TABLE access_key
		(
		  ...
		  lat_str real, lat_end real, lon_str real, lon_end real,ramp_id text
		)
    '''
...
insert_table = '''
COPY access_key 
FROM '%s\\%s\\access_key.csv' 
DELIMITER ',' CSV; 
COPY access_pic 
FROM '%s\\%s\\access_pic.csv' 
DELIMITER ',' CSV; 
COPY access_valuelaser 
FROM '%s\\%s\\access_valuelaser.csv' 
DELIMITER ',' CSV; 
COPY access_distress_pic
FROM '%s\\%s\\access_distress_pic.csv' 
DELIMITER ',' CSV; ''' % (path,a,path,a,path,a,path,a)
...
print("STEP3:create table data_suvey successfully")
print('')
conPG.commit()
...
str_end = '''
		SELECT event_str, event_end,link_id, km_start::text, km_end::text,lane_no::text, lat_str::numeric(8,4), 
...
create table survey as --delete from survey_local
	select
	row_number() over (order by chainage_str::int,link_id)+1 as survey_id, null::int as subsection_id, section_id, link_id,
...
step_11 = '''create table data_pave_suvey as 
	...
	,b.km_start,b.km_end, '%s' as filename,b.run_code,b.survey_code
	from
		(select a.event_str,a.event_end,a.link_id,b.chainage_pic,b.frame_number,b.lon,b.lat,a.lat_str,a.lat_end,a.lon_str,a.lon_end,
		 st_setsrid(st_makepoint(b.lon::real ,b.lat::real),4326) as the_geom,a.survey_date
		from
			(select event_str,event_end,event_name,link_id,survey_date::date,lat_str,lat_end,lon_str,lon_end from data_suvey
			group by event_str,event_end,event_name,link_id,survey_date,lat_str,lat_end,lon_str,lon_end
			order by event_str) a
		left join  access_distress_pic b on b.chainage_pic between a.event_str and a.event_end
		order by b.chainage_pic) a 
	left join survey b on a.link_id = b.link_id and a.survey_date = b.date 
	order by a.chainage_pic''' %(a)
...
from
	(select survey_id,link_id,filename||'-PAVE-0-'||case when length(frame_number::text) = 1 then '0000'||(frame_number::int+1)::text
